...
function createPost({ idusuario, nombre, autor, contenido, fecha }) {
  const newPost = {
    id: publicaciones.length ? publicaciones[publicaciones.length - 1].id + 1 : 1,
    idusuario: idusuario || 'anonymous',
    autorId: idusuario || 'anonymous',
    nombre,
    autor,
    contenido,
...
function savePost({ idusuario, idpublicacion }) {
  const exists = savedPosts.find(
    saved => saved.idpublicacion === idpublicacion && saved.idusuario === idusuario
  );
  if (!exists) {
    savedPosts.push({ idusuario, idpublicacion });
  }
  return exists || savedPosts[savedPosts.length - 1];
}

function getSavedPostsForUser(idusuario) {
  const ids = savedPosts
    .filter(saved => saved.idusuario === idusuario)
    .map(saved => saved.idpublicacion);
  return publicaciones.filter(post => ids.includes(post.id));
}
